...
class OmniStereoDataset(Dataset):
    def __init__(self, root_dir, filename_txt, transform=None, fov=220):
        ...
        # Khởi tạo Ocam để lấy vùng valid (mask) cho từng camera
        self.ocams = []
        self.valids = []
        for cam in self.cam_list:
            ocam_file = join(root_dir, f'o{cam}.txt')
    ...
    def __getitem__(self, idx):
        ...
        # Load images cho 4 camera
        for i, cam in enumerate(self.cam_list):
            img_path = join(self.root_dir, cam, filename)
            # Sử dụng hàm load_image ở bên dưới
            sample[cam] = load_image(img_path, gray=False, valid=self.valids[i])

        # Load GT inverse depth
        depth_path = join(self.root_dir, self.depth_folder, filename)
        sample['idepth'] = load_invdepth(depth_path)
...
# --- CÁC HÀM HELPER (Cần thiết để __init__.py không báo lỗi) ---

def load_image(filename, gray=False, valid=None):
    """Đọc ảnh và áp dụng mặt nạ valid của camera mắt cá"""
    img = cv2.imread(filename)
    if img is None:
        # Trả về ảnh đen nếu không tìm thấy file
        return np.zeros((480, 500, 3), dtype=np.uint8) if not gray else np.zeros((480, 500), dtype=np.uint8)
...
def load_invdepth(filename, min_depth=0.55):
    """Đọc ảnh depth 16-bit và chuyển sang đơn vị inverse depth (1/m)"""
    invd_value = cv2.imread(filename, cv2.IMREAD_ANYDEPTH)
    if invd_value is None:
        return np.zeros((256, 512), dtype=np.float32)
    
    invd_value = invd_value.astype(np.float32)
    # Công thức chuẩn hóa cho OmniThings
