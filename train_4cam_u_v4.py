...
def train(args, model, train_loader, optimizer, writer, epoch, device):
    ...
    for idx, batch in enumerate(pbar):
        ...
        # --- Normalize depth ---
        batch['idepth'] = batch['idepth'] / (batch['idepth'].max() + 1e-8)
        pred = model(batch)
        ...
        if idx % args.log_interval == 0:
            ...
            # Debug pred vs gt
            print("Pred idx:", pred[0].detach().cpu().numpy().flatten()[:10])
            print("GT idx:", gt_idx[0].detach().cpu().numpy().flatten()[:10])
...
def main():
    ...
    # Subset demo (min 64 images)
    subset_size = max(math.ceil(len(trainset)/100), 64)
